...
Como las propiedades de `Column` estan tipadas (`string $label`, `array $rules`, `bool $hidden`, etc.), la asignacion directa (`$column->$property = $value`) puede lanzar un `\TypeError` si el override tiene la forma equivocada (ej. `['label' => null]` o `['rules' => 'required']` en vez de `['required']`). `applyTo()` captura ese `\TypeError` y relanza un `\InvalidArgumentException` con el nombre de la columna, la propiedad y el tipo recibido — un error de configuracion del integrador debe fallar con un mensaje que apunte a la causa, no con un `TypeError` generico apuntando a una linea interna de la libreria.

### Bug real: la introspeccion de PostgreSQL no filtraba por `table_schema`

`TableIntrospector::introspectPgsql()` consultaba `information_schema.columns` filtrando solo por `table_name`, sin `table_schema`. En una base con la misma tabla en mas de un schema (ej. `public.usuarios` y `auditoria.usuarios`), las columnas podian mezclarse entre ambas sin ningun error visible — un `TableSchema` incorrecto, silencioso. La introspeccion de MySQL ya evitaba el equivalente (`table_schema = DATABASE()`); Postgres ahora filtra por `table_schema = current_schema()` tanto en la consulta principal como en el join que resuelve la llave primaria.
...
## `Crud\UpdateChecker`: aviso de nueva version, opt-in y fail-safe

`AppyCrud` guarda su propia version en la constante `AppyCrud::VERSION` (string, actualizada a mano en cada tag/release — no se lee de `composer.lock` ni de ningun otro archivo, para que funcione igual en instalaciones sin Composer). Solo se compara contra Packagist si la opcion `'checkForUpdates' => true` esta activa; por default la libreria no hace ninguna peticion de red por su cuenta.

`UpdateChecker::check($currentVersion, $cacheDir = null)` es un metodo estatico sin estado propio (no requiere instanciar nada, ni conexion a BD): consulta `https://repo.packagist.org/p2/appylogi/appycrud.json` (el mismo endpoint que ya se uso para verificar manualmente que el paquete estuviera disponible tras cada release), filtra las versiones que no son un tag semver real (descarta `dev-master` y similares con una regex `^v?\d+\.\d+\.\d+$`), y se queda con la mayor via `version_compare()`. El resultado se cachea en un archivo JSON (`sys_get_temp_dir() . '/appycrud-update-check.json'` por default, pero recibe `$cacheDir` explicito para poder testear sin pisar el cache real) por 24 horas — incluyendo el caso de fallo de red, para no reintentar en cada peticion si Packagist esta caido.

**Todo el metodo esta pensado para nunca lanzar una excepcion**: `httpGet()` prueba `curl` primero y cae a `file_get_contents()` con `allow_url_fopen` si no hay `curl`, y si ninguno esta disponible retorna `null` sin mas. Cualquier error de red, JSON invalido, o timeout (2 segundos, via `CURLOPT_TIMEOUT`/`stream_context`) se traduce en `check()` devolviendo `null` — el llamador (`AppyCrud::availableUpdate()`) nunca necesita un `try/catch`.

`AppyCrud::renderList()` (la pagina completa) le pasa el resultado a `TailwindRenderer::renderList()` como ultimo parametro opcional (`?array $updateInfo`); `renderListBody()` (el fragmento usado en el filtrado AJAX) **no** lo recibe a proposito — el aviso solo debe aparecer en la carga inicial de la pagina, no reaparecer cada vez que el usuario filtra o cambia de pagina. El descarte se guarda en `localStorage` del navegador (`appycrudDismissUpdateBanner()`, definida junto al resto de JS global en `renderModalShell()`) keyeado por numero de version — asi, si sale una version aun mas nueva despues de que el usuario ya descarto el aviso anterior, vuelve a aparecer.
